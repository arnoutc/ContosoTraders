...
class LoginComponent extends Component {
    ...
    handleFormErrors() {
        this.props.enqueueSnackbar("Username or Password can not be empty", {
            variant: 'error',
    ...
    handleLoginB2CClick = async (e) => {
        ...
        try {
            ...
            this.props.enqueueSnackbar("Cannot not be logged", {
                variant: 'error',
                anchorOrigin: { vertical: 'top', horizontal: 'center' },
                autoHideDuration: 6000,
            });
    ...
    render() {
        const { t } = this.props;
        // Access t from props (withTranslation)
        return (
            <div className={this.state.isModalOpened ? 'modal-overlay is-opened' : 'modal-overlay'}>
                {/* Replace Alert with notistack's snackbar in your wrapper */}
                <div className="modal">
...
// Wrapper to inject enqueueSnackbar from notistack's useSnackbar
function LoginComponentWrapper(props) {
    const { enqueueSnackbar } = useSnackbar();
    return <LoginComponent {...props} enqueueSnackbar={enqueueSnackbar} />;
}

export default withTranslation()(LoginComponentWrapper);