...
// CLASS COMPONENT (withTranslation injected)
class Detail extends Component {
  ...
  getMeetingDetails = async (isVideo) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        this.navigateTo("/meeting" + response.data.joinUrl);
      });
  };
  navigateTo = (location) => {
    // Use navigate hook passed as prop from wrapper
  ...
  render() {
    ...
    const { loadingRelated, loggedIn, t } = this.props;
    return (
...
// WRAPPER FUNCTION TO INJECT ROUTER HOOKS (for navigation)
function DetailWrapper(props) {
  const navigate = useNavigate();
  const location = useLocation();
  const params = useParams();
  // Pass t/i18n from withTranslation by spreading props
  return (
    <Detail
      {...props}
      navigate={navigate}
...
// Default export: withTranslation wraps the wrapper
export default withTranslation()(DetailWrapper);